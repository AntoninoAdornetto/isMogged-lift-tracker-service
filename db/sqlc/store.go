...
type CreateNewLiftWithExersiseReq struct {
	ExersiseName string    `json:"exersise_name"`
...
type CreateNewLiftWithExersiseRes struct {
	Lift     Lift      `json:"lift"`
	SetId    uuid.UUID `json:"set"`
	Exersise Exersise  `json:"exersise"`
}

// TODO: Write unit test for this tx
func (store *Store) CreateNewLiftWithNewExersise(ctx context.Context, args CreateNewLiftWithExersiseReq) (CreateNewLiftWithExersiseRes, error) {
	var res CreateNewLiftWithExersiseRes

	err := store.execTx(ctx, func(q *Queries) error {
		var err error

		res.Exersise, err = q.CreateExersise(ctx, CreateExersiseParams{
			ExersiseName: args.ExersiseName,
